...
import { submissionMethodService } from '../../../services/workService'
...
export default function AdminDeliveryMethodsPage() {
  ...
  const create = useMutation({
    mutationFn: () => submissionMethodService.create({ name: newName }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['delivery-methods'] }); setNewName('') },
    onError: () => setError('Failed to create method'),
  })

  const update = useMutation({
    mutationFn: () => submissionMethodService.update(editId, { name: editName }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['delivery-methods'] }); setEditId(null) },
    onError: () => setError('Failed to update method'),
  })

  const remove = useMutation({
    mutationFn: (id) => submissionMethodService.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['delivery-methods'] }),
    onError: () => setError('Failed to delete method'),
  })
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold mb-6">Delivery Methods</h1>
      ...
      <div className="space-y-2">
        {methods?.map(method => (
          <div key={method.id} className="flex items-center justify-between bg-slate-900 border border-white/5 rounded-xl px-4 py-3">
            {editId === method.id ? (
              <input
                type="text"
                value={editName}
            ...
            <div className="flex gap-3">
              {editId === method.id ? (
                ...
                <>
                  <button onClick={() => { setEditId(method.id); setEditName(method.name) }} className="text-xs text-slate-400 hover:text-white transition-colors">Edit</button>
                  <button onClick={() => remove.mutate(method.id)} className="text-xs text-red-400 hover:text-red-300 transition-colors">Delete</button>
                </>
              )}
            </div>
          </div>
